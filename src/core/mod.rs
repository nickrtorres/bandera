...
#[derive(Debug)]
pub struct Vm<H: interrupt::Handler + Default> {
    ax: FullViewRegister,
    bx: FullViewRegister,
    bp: u16,
    cf: u8,
...
impl<H: interrupt::Handler + Default> AbstractMachine for Vm<H> {
    fn update_reg(&mut self, dst: RegisterTag, src: RegisterTag) {
...
impl<H: interrupt::Handler + Default> Vm<H> {
    pub fn new() -> Self {
        Vm {
